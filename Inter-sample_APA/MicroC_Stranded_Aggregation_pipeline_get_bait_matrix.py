# This script creates an APA matrix centered on a single bite
...
for n in preys.index:
	...
	if (baitStrand == '+') and (preys['strand'][n] == '+'):
		...
		while a > region - winSize:
			...
			a -= step
		aggregate = sum([aggregate, aggregatePair])
# Invert the DF index to allow bottom-to-top increase positions relative to the TSS and save the matrix
